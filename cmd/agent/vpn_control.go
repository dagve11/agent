...
type agentVPNSharedExitFailedSession struct {
	sessionID string
...
func (m *AgentVPNManager) Start(req model.VPNControlRequest) (model.VPNControlResult, error) {
	...
	m.mu.Unlock()
	if session.sharedExitRuntimeKey == "" {
		m.watchSidecar(req.SessionID, session, sidecar)
	}
	if req.RelayMode == model.VPNRelayModeDirect {
		m.watchBridge(req.SessionID, session, bridge)
	}
...
func (m *AgentVPNManager) stopTrackedSession(req model.VPNControlRequest, failOnTunRestore bool, cleanupCore bool, clearSystemProxyBeforeRestore bool) ([]string, error) {
	...
	if clearSystemProxyBeforeRestore && systemProxyWasApplied {
		if m.hasActiveSystemProxySession(session) {
			logs = append(logs, "[cleanup] system_proxy_clear=skipped: active-session")
		} else if clearErr := m.clearSessionSystemProxy(); clearErr != nil {
...
func (m *AgentVPNManager) markSharedExitRuntimeFailed(key string, err error) {
	...
	delete(m.sharedExitRuntimes, key)
	sessions := make([]agentVPNSharedExitFailedSession, 0, len(runtime.refs))
	for sessionID := range runtime.refs {
		session := m.sessions[sessionID]
		if session == nil || session.sharedExitRuntimeKey != key {
			continue
		}
		sessions = append(sessions, agentVPNSharedExitFailedSession{
			sessionID: sessionID,
			session:   session,
		})
	}
	...
	for _, failed := range sessions {
		m.markSessionFailedWithReasonAndLogsIfCurrent(failed.sessionID, failed.session, err, model.VPNFailureReasonUnknown, cleanupLogs)
	}
}
...
func (m *AgentVPNManager) watchSidecar(sessionID string, session *AgentVPNSession, sidecar *AgentVPNSidecar) {
	if sidecar == nil {
		return
	}
	go func() {
		...
		m.markSessionFailedIfCurrent(sessionID, session, err)
	}()
}

func (m *AgentVPNManager) watchBridge(sessionID string, session *AgentVPNSession, bridge *AgentVPNBridge) {
	...
	go func() {
		...
		m.markBridgeFailed(sessionID, session, fmt.Errorf("VPN bridge relay closed: %w", err), vpnBridgeFailureReason(err))
	}()
}

func (m *AgentVPNManager) markBridgeFailed(sessionID string, expected *AgentVPNSession, err error, reason string) {
	if err == nil {
		return
	}
	m.mu.Lock()
...
func (m *AgentVPNManager) CleanupStaleSessions() {
	...
	for _, state := range states {
		...
		if state.SystemProxyApplied && m.systemProxyManager != nil {
			if activeReq, ok := m.activeSystemProxySession(nil); ok {
				if err := m.systemProxyManager.Apply(activeReq.ListenHTTP, activeReq.ListenSOCKS); err != nil {
					printf("VPN active system proxy reapply failed during stale cleanup for session %s: %v", state.SessionID, err)
					continue
				}
...
func (m *AgentVPNManager) restoreSessionSystemProxy(session *AgentVPNSession) (vpnSystemProxyRestoreResult, error) {
	...
	if activeReq, ok := m.activeSystemProxySession(session); ok {
		if err := m.systemProxyManager.Apply(activeReq.ListenHTTP, activeReq.ListenSOCKS); err != nil {
			return vpnSystemProxyRestoreNone, fmt.Errorf("reapply active VPN system proxy for session %s: %w", activeReq.SessionID, err)
		}
		session.systemProxyApplied = false
...
func (m *AgentVPNManager) hasActiveSystemProxySession(exclude *AgentVPNSession) bool {
	_, ok := m.activeSystemProxySession(exclude)
	return ok
}

func (m *AgentVPNManager) activeSystemProxySession(exclude *AgentVPNSession) (model.VPNControlRequest, bool) {
	if m == nil {
		return model.VPNControlRequest{}, false
	}
	m.mu.Lock()
	defer m.mu.Unlock()
